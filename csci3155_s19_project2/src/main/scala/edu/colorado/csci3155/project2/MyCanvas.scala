...
case class Polygon(val cList: List[(Double, Double)]) extends Figure {
    ...
    override def getBoundingBox: (Double, Double, Double, Double ) = {(cList.map(x=>x._1).min, cList.map(x=>x._1).max, cList.map(y=>y._2).min, cList.map(y=>y._2).max)}

    //TODO: Create a new polygon by shifting each vertex in cList by (x,y)
    //    Do not change the order in which the vertices appear
    override def translate(shiftX: Double, shiftY: Double): Polygon = {Polygon(cList.map(t=>(t._1+shiftX, t._2+shiftY)))}

    def rotate(angRad: Double): Polygon = {Polygon(cList.map(t=>(t._1*Math.cos(angRad)-t._2*Math.sin(angRad), t._1*Math.sin(angRad)+t._2*Math.cos(angRad))))}
...
case class MyCircle(val c: (Double, Double), val r: Double) extends Figure {
    ...
    def rotate(angRad: Double): MyCircle = {MyCircle((c._1*Math.cos(angRad)-c._2*Math.sin(angRad),c._1*Math.sin(angRad)+c._2*Math.cos(angRad)), r)}
...
class MyCanvas (val listOfObjects: List[Figure]) {
    ...
    def getBoundingBox: (Double, Double, Double, Double) = {
        (listOfObjects.map(p => p.getBoundingBox).map(t => t._1).min, listOfObjects.map(p => p.getBoundingBox).map(t => t._2).max, listOfObjects.map(p => p.getBoundingBox).map(t => t._3).min, listOfObjects.map(p => p.getBoundingBox).map(t => t._4).max)
    }
    //TODO: Write a function to translate each figure in the canvas by shiftX, shiftY
    def translate(shiftX: Double, shiftY: Double): MyCanvas = new MyCanvas(listOfObjects.map(t=>t.translate(shiftX, shiftY)))
    ...
    def placeRight(myc2: MyCanvas):MyCanvas = {
        ...
        var shiftX = c1._2-c1._1;
        var shiftY = (c1._4-c1._3)/2 - (c2._4-c2._3)/2;
        var c2prime = myc2.translate(shiftX, shiftY).listOfObjects;
        return this.overlap(new MyCanvas(c2prime));
    }
    ...
    def rotate(angRad: Double): MyCanvas = {
        return new MyCanvas(listOfObjects.map(o=> o match{
